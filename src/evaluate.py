...
def eval_contrastive_wrapper(args):
  ...
  if args.embedding_model_path:
    embedding_model.load(args.embedding_model_path)
    if args.index_path:
      index = faiss.read_index(args.index_path)
    else:
      model_name = os.path.basename(args.embedding_model_path).split('.')[0]
...
def eval_softmax_wrapper(args):
  test_dataset = LiarDataset("test", num_labels=num_labels)
  test_ldr = DataLoader(test_dataset, batch_size=10)
  embedding_model = DistilBertForSequenceEmbedding()  
  embedding_model.load(args.embedding_model_path)

  prediction_model = SoftmaxHead(input_length=embedding_model.embedding_size, num_classes=num_labels)
  prediction_model.load(args.prediction_model_path)

  eval_softmax(embedding_model, prediction_model, test_ldr)
...
def eval_contrastive(embedding_model: DistilBertForSequenceEmbedding, index: faiss.IndexIDMap, prediction_model: WeightedMajorityVoter, K: int, id_map, dataloader: DataLoader):
  ...
  logging.info(f"Eval (contrastive) accuracy: {metrics['accuracy']}")
  logging.info(f"Eval (contrastive) weighted f1_score: {metrics['f1_score']}")

  return metrics

def eval_softmax(embedding_model: DistilBertForSequenceEmbedding, prediction_model: SoftmaxHead, dataloader: DataLoader):
  if torch.cuda.is_available():
    logging.info("GPU available!")
    embedding_model.to('cuda')
  ...
  with torch.no_grad():
    for (batch_idx, batch) in tqdm(enumerate(dataloader)):
      # Generate embeddings
      embeddings = embedding_model(batch["data"]) 
      logits = prediction_model(embeddings)
  ...
  logging.info(f"Eval (softmax) accuracy: {metrics['accuracy']}")
  logging.info(f"Eval (softmax) weighted f1_score: {metrics['f1_score']}")
...
if __name__=="__main__":
    ...
    if args.contrastive and (args.embedding_model_path or args.artifact):
      eval_contrastive_wrapper(args)
    elif args.embedding_model_path and args.prediction_model_path:
      eval_softmax_wrapper(args)
    else:
      raise Exception("Invalid configuration options.")
