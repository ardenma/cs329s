...
def get_model_path_from_artifact(artifact_name: str) -> str:
    # Create artifacts dir if it doesn't already exist
    artifacts_dir = os.path.join(pathlib.Path(__file__).parent.parent.resolve(), "artifacts")
    ...
    api = wandb.Api()
    artifact = api.artifact(f'cs329s/cs329s/{artifact_name}', type='distilbert-embedding-model')
    model_name = "_".join(artifact_name.split('_')[0:3])
