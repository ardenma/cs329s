...
class LiarDataset(Dataset):
    def __init__(self, split: str = "train", num_labels: int = 3):
        ...
        assert split in (
            ...
            "validation",
            "test",
        ), f"Unrecognized data split '{split}'"
    ...
    def __getitem__(self, idx: int) -> Dict[str, Union[int, float]]:
        ...
        return {
            "data": data,
            "label": label,
            "id": string_to_id(self.dataset[idx]["id"]),
        }
    ...
    def get_class_balance(
        ...
        class_balance = np.array(
            list(map(lambda x: x / len(self.dataset), class_balance))
        )
        if as_tensor:
    ...
    def get_class_weight_for_train(
        self, as_tensor: bool = False
    ) -> Union[np.array, torch.tensor]:
        class_balance = np.zeros(self.num_labels)
        for ex in self.dataset:
        ...
        inv_class_balance = np.array(
            list(map(lambda x: len(self.dataset) / x, class_balance))
        )
        if as_tensor:
