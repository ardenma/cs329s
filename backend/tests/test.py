...
def main(args):
    if args.test_name == "throughput":
        ...
        test_latency_under_load(
            num_queries=args.n, server_endpoint=args.server_endpoint
        )
    else:
...
def test_latency_under_load(num_queries: int, server_endpoint: str) -> Dict[str, float]:
    ...
    for i in range(num_queries):
        test_results = send_parallel(
            data=[data_samples[i]], server_endpoint=server_endpoint
        )
        e2e_latency_ms = test_results["e2e_delay"] * 1000
        server_side_latency_ms = test_results["results"][0]["diagnostics"][
    ...
    print(
        f"Avg. server-side inference latency: {mean_server_side_latency_ms:.3f} ms/request."
    )
