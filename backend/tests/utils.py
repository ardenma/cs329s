...
@ray.remote
def send_query(
    query_id: int, data: str = "asdfjkl", server_endpoint: str = "127.0.0.1:8000"
) -> requests.Response:
...
def send_parallel(data: List[str], server_endpoint: str = "127.0.0.1:8000"):
    ...
    results = ray.get(
        [send_query.remote(i, d, server_endpoint) for i, d in enumerate(data)]
    )
...
def sample_test_dataset(
    ...
    samples = [
        dataset[int(rand_idx)]
        for rand_idx in np.random.randint(0, len(dataset), size=num_samples)
    ]
    return samples
