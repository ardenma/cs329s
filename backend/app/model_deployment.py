...
class embedding_model:
    def __init__(self, artifact_name: str = None):
        ...
        self.num_labels = int(
            artifact_name.split("-")[3]
        )  # TODO save this info with model
    ...
    async def batch_handler(self, inputs: List[str]) -> List[torch.tensor]:
        ...
        logging.debug(
            f"Embedding model called with batch of {len(inputs)} inputs: {inputs}, generated corresponding embedding: {batched_embeddings}"
        )
...
class prediction_model:
    def __init__(self, artifact_name: str = None, K: int = 3):
        ...
        logging.info(
            f"Index for prediction model loaded from artifact '{artifact_name}'."
        )
    ...
    async def batch_handler(
        ...
        batched_similarities, batched_ids = self.index.search(
            batched_embeddings.cpu().numpy(), self.K
        )
        ...
        logging.debug(
            f"Prediction model called with batch of {len(embeddings)} embeddings: {embeddings}, generated corresponding prediction: {batched_predictions}"
        )
        ...
        batched_results = [
            ...
            for pred, ids, similarities in zip(
                batched_predictions, batched_ids, batched_similarities
            )
        ]
...
class MisinformationDetectionModel:
    def __init__(self, config: AppConfig):
        ...
        embedding_model.options(
            num_replicas=config.num_embedding_model_replicas
        ).deploy(artifact_name=config.artifact_name)
        prediction_model.options(
            num_replicas=config.num_prediction_model_replicas
        ).deploy(artifact_name=config.artifact_name)
