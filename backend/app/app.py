...
from utils.datatypes import ModelInfoResponse, Query, Response, Feedback, PredictionFeedback, FeedbackResponse, AppConfig
...
class MisinformationDetectionApp:
    ...
    async def predict(self, query: Query) -> Response:
        ...
        diagnostics = {"server_side_latency_ms": server_side_latency_ms, "model_name_and_version": self.name}
        response = Response(
                    id=query.id, 
    ...
    async def log_prediction_feedback(self, feedback: PredictionFeedback) -> FeedbackResponse:
        with open(os.path.join(LOGDIR, f"deployment_log-{self.deployment_time}.txt"), "a+") as logfile:
            logfile.write(f"({self.name}) {datetime.now()}: for query '{feedback.query}' predicted '{feedback.predicted_class}' user suggested label '{feedback.user_suggested_class}'\n")
        logging.info(f"Received prediction feedback: {feedback}")
        return FeedbackResponse(ack=True)

    @app.get("/model_info", response_model=ModelInfoResponse)
    async def get_model_info(self) -> ModelInfoResponse:
        if (self.name == "daily-tree-15-3-labels:v5"):  # TODO save more metadata with artifacts and fix hardcoding
            response = ModelInfoResponse(model_name=self.name.split(':')[0], model_version=self.name.split(':')[1], model_test_f1_score=0.414)
        else:
            response = ModelInfoResponse(model_name=self.name.split(':')[0], model_version=self.name.split(':')[1])
        return response
