...
class StreamlitFrontend:
    ...
    def render_blurb(self):
        st.markdown(
            "Hi and welcome to our **Misinformation Detection App**! Try submitting a query above, or click on the below box for more information about our app and model!"
        )
        with st.expander(label="More Information / Technical Details:", expanded=False):
            ...
            st.markdown(
                "3. Then, given a query we can generate and embedding, match it to the K-closest embeddings in our embedding space, \
                        and then utilize a voting based approach to generate inferences (e.g. if 2/3 votes are 'true' we will return the label true!)"
            )
            st.markdown(
                "See our github for the full source code: https://github.com/ardenma/cs329s"
            )
    ...
    def render_result(self):
        ...
        with st.container():
            ...
            if "current_model_name" in st.session_state:
                col1.write(
                    f"Current model and version: {st.session_state.current_model_name}:{st.session_state.current_model_version}"
                )
                col1.write(
                    f"Current model F1 score on 3 label LIAR dataset test split: {st.session_state.current_model_test_f1:.3f}"
                )
            col1.button(label="request_model_info", on_click=self.request_model_info)

            with col2.form("feedback_form "):
                st.radio(
                    label="Think we got something wrong? Please suggest a new class label!",
                    key="user_suggested_label",
        ...
        with st.container():
            ...
            col1.metric(
                ...
                f"{st.session_state.last_latency_ms:.3f}",
                f"{st.session_state.last_latency_delta_ms:.3f}",
            )
            col2.metric(
                "Inference Latency (ms)",
                f"{st.session_state.last_server_latency_ms:.3f}",
                f"{st.session_state.last_server_latency_delta_ms:.3f}",
            )
            col3.metric(
                "Network Latency (ms)",
                f"{st.session_state.last_network_latency_ms:.3f}",
                f"{st.session_state.last_network_latency_delta_ms:.3f}",
            )
        ...
        for i in range(3):
            with st.container():
                ...
                col3.metric(
                    "Cosine Similarity", f"{resp['example_similarities'][i]:.3f}"
                )
    ...
    def send_query(self):
        query = Query(
            id=st.session_state.current_query_id, data=st.session_state.query
        ).json()
        ...
        with st.spinner("Sending query to server. Please wait..."):
            st.session_state.current_response = requests.post(
                f"http://{self.server_endpoint}/app/predict", data=query
            ).json()
        time_end = perf_counter()

        current_latency_ms = (time_end - time_start) * 1000
        current_server_latency_ms = st.session_state.current_response["diagnostics"][
        ...
        if "last_latency_ms" in st.session_state:
            st.session_state["last_latency_delta_ms"] = (
        ...
        if "last_server_latency_ms" in st.session_state:
            st.session_state["last_server_latency_delta_ms"] = (
        ...
        if "last_network_latency_ms" in st.session_state:
            st.session_state["last_network_latency_delta_ms"] = (
    ...
    def send_feedback(self):
        ...
        with st.spinner(
            "Thanks for the feedback! Sending feedback to server. Please wait..."
        ):
            requests.post(
                f"http://{self.server_endpoint}/app/feedback", data=feedback
            ).json()
    ...
    def send_prediction_feedback(self):
        feedback = PredictionFeedback(
            ...
            predicted_class=st.session_state.current_response["predicted_class"],
            user_suggested_class=st.session_state.user_suggested_label,
        ).json()
        with st.spinner(
            "Thanks for the feedback! Sending feedback to server. Please wait..."
        ):
            requests.post(
                f"http://{self.server_endpoint}/app/prediction_feedback", data=feedback
            ).json()
        st.success("Thanks! Your feedback was received :)")
    ...
    def request_model_info(self):
        with st.spinner("Requesting model info from server. Please wait..."):
            response = requests.get(
                f"http://{self.server_endpoint}/app/model_info"
            ).json()
...
if __name__ == "__main__":
    ...
    if not args.server_endpoint:
        ...
        logging.warning(
            f"No server endpoint specified, connecting to server at {args.server_endpoint}."
        )
